...
CudaExperiment::CudaExperiment(const unsigned int minCu, const unsigned int maxCU, const std::string &name,
	const std::string &description) : Experiment(minCu, maxCU, name, description){
  running = false;
  should_run = false;
}
...
void CudaExperiment::Init2(bool batch){
	if (!batch) {
		...
		while (selectedDev != 0 && selectedDev != 1) {
			...
			for (size_t i = 0; i < cuda::total_num_devices; i++) {
				cout << "\t" << i + 2 << "\t" << cuda::devices[i].short_name
					<< "\t CU:" << cuda::devices[i].computeUnits;
				if (selected[i] == true) {
