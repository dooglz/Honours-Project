...
__global__ void bit_reduce_count(const uint32_t *input_array, uint32_t *intBuf, uint32_t *countBuf,
  ...
  if ((threadIdx.x + sharedMem[threadIdx.x] - size) == 149){
	  printf("\n\n%i, %i\n", threadIdx.x - 2, sharedMem[threadIdx.x - 2]);
	  printf("%i, %i\n", threadIdx.x-1, sharedMem[threadIdx.x-1]);
	  printf("%i, %i, %i\n", threadIdx.x, sharedMem[threadIdx.x], size);
	  printf("%i, %i\n", threadIdx.x+1, sharedMem[threadIdx.x+1]);
	  printf("%i, %i\n", threadIdx.x + 2, sharedMem[threadIdx.x + 2]);
  }
  if (a <= 0xff) {
...
__global__ void count_compact(uint8_t *intBuf, const uint16_t dataSize, uint32_t *countBuf, uint32_t *sizeBuf) {
	uint16_t readIndex = 0;
	uint16_t writeIndex = 0;
	for (uint16_t i = 0; i < dataSize; ++i) {
		const uint16_t readIndex = i * 4;
