...
void D3D12nBodyGravity::CreateAsyncContexts() {
  ...
  ThrowIfFailed(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_COMPUTE, m_computeAllocator.Get(), nullptr,
 ...
 /* m_threadHandles = CreateThread(nullptr, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(ThreadProc), reinterpret_cast<void *>(&m_threadData),
...
void D3D12nBodyGravity::DoCompute() {
...

  {
    ...
    pCommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(pUavResource, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
...
void D3D12nBodyGravity::PopulateCommandList()
{
	...
	float viewportHeight = static_cast<float>(static_cast<UINT>(m_viewport.Height));
	float viewportWidth = static_cast<float>(static_cast<UINT>(m_viewport.Width));

		const UINT srvIndex = 0 + (m_srvIndex == 0 ? SrvParticlePosVelo0 : SrvParticlePosVelo1);
...
DWORD D3D12nBodyGravity::AsyncComputeThreadProc()
{
	ID3D12CommandQueue* pCommandQueue = m_computeCommandQueue.Get();
	ID3D12CommandAllocator* pCommandAllocator = m_computeAllocator.Get();
	ID3D12GraphicsCommandList* pCommandList = m_computeCommandList.Get();
...
void D3D12nBodyGravity::OnDestroy()
{
	...
	WaitForSingleObject(m_threadHandles, INFINITE);
